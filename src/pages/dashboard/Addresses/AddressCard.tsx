...
export const AddressCard: React.FC<AddressCardProps> = ({
  ...
  return (
    <div
      className={`border bg-white/60 dark:bg-white/[0.03] p-5 space-y-3 transition-colors rounded-2xl ${
        addr.isDefault
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          ...
          {addr.isDefault && (
            <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 font-bold bg-gold/15 text-gold border border-gold/40 flex items-center gap-1 rounded-full">
        ...
        {addr.digitalAddress && (
          <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 font-bold bg-black/5 dark:bg-white/10 text-black/60 dark:text-white/60 border border-black/10 dark:border-white/15 rounded-full">
            {addr.digitalAddress}
          </span>
        )}
      </div>
